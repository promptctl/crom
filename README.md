...
| `features` | `[defaults]`, `[profiles.X]` | Chrome features to turn on or off, as `FeatureName = true`/`false`. A name no layer mentions is left alone. crom folds every layer's table into a single `--enable-features` and `--disable-features`, so a feature you turn off joins crom's own rather than replacing them — which is why `flags` may not name those two switches itself. |
